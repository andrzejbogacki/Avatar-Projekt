...
- **Zależności zewnętrzne (stan 2026-07-08):** `sweph` 2.10.3-7 zainstalowany (`backend/package.json`); pliki efemeryd DE440/441 lub `.se1` — NIEDOSTARCZONE (bez nich kalkulator zwraca jawny błąd, zakaz fallbacku Moshiera); Redis nieobecny w środowisku — bufor działa na fallbacku in-memory z odnotowanym powodem; API: NOAA SWPC, NOAA Kp (dane GFZ), stacja Rezonansu Schumanna (O5 — endpoint niezdefiniowany, źródło zgłasza jawny brak). Brak sieci lub bufora nie blokuje obliczeń — ostatni stan stemplowany.
